...
from src.design_system import COLORS, TYPOGRAPHY, SPACING, SHADOWS, BORDER_RADIUS, ANIMATION


def kpi_card(title, value, change=None, icon=None, is_positive=True):
    """Premium KPI Card Component"""
    change_color = "#4CAF50" if is_positive else "#F44336"
    change_html = f'<p style="color:{change_color}; font-size:{TYPOGRAPHY["caption"]["size"]};margin:0;">{change}</p>' if change else ''
    icon_html = f'<div style="font-size:24px;">{icon}</div>' if icon else ''
    st.markdown(
        f"""
...
def metric_counter(value, label, is_positive=True):
    ...
    st.markdown(
        ...
        <div style="text-align:center; padding:{SPACING['md']};">
            <div style="font-size:32px; font-weight:{TYPOGRAPHY['h1']['weight']};color:{COLORS['dark']['accent']};">
                {value}
            </div>
            <div style="color:{COLORS['dark']['text_secondary']};font-size:{TYPOGRAPHY['caption']['size']};">
                {label}
            </div>
        </div>
        """, unsafe_allow_html=True
...
def card_header(title, subtitle=None):
    ...
    subtitle_html = f'<p style="color:{COLORS["dark"]["text_secondary"]};font-size:{TYPOGRAPHY["body"]["size"]};margin:0;">{subtitle}</p>' if subtitle else ''
    st.markdown(
        f"""
