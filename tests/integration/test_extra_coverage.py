...
from ml_pipeline_monitor.core.secrets import SecretsManager, get_secret
from ml_pipeline_monitor.services.pipeline_service import compute_next_run_ts, get_pipeline_defaults, should_trigger_scheduled_run
...
class TestPipelineServiceMore:
    ...
    def test_should_trigger_scheduled_run_enabled(self):
        ...
        assert should_trigger_scheduled_run(enabled=True, next_run_at=datetime.now(timezone.utc) - timedelta(minutes=10))

    def test_should_trigger_scheduled_run_disabled(self):
        from datetime import datetime, timezone
        assert not should_trigger_scheduled_run(enabled=False, next_run_at=datetime.now(timezone.utc))
