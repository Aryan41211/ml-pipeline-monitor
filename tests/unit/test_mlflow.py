...
def test_is_enabled_true():
    with patch("ml_pipeline_monitor.ml.mlflow_tracker.load_config", return_value={"mlflow": {"enabled": True}}):
        assert _is_enabled() is True


def test_is_enabled_false():
    with patch("ml_pipeline_monitor.ml.mlflow_tracker.load_config", return_value={"mlflow": {"enabled": False}}):
        assert _is_enabled() is False


def test_is_enabled_missing():
    with patch("ml_pipeline_monitor.ml.mlflow_tracker.load_config", return_value={}):
        assert _is_enabled() is False


def test_log_pipeline_run_disabled():
    with patch("ml_pipeline_monitor.ml.mlflow_tracker._is_enabled", return_value=False):
        with patch("ml_pipeline_monitor.ml.mlflow_tracker.load_config", return_value={"mlflow": {}}):
...
def test_log_pipeline_run_exception():
    with patch("ml_pipeline_monitor.ml.mlflow_tracker._is_enabled", return_value=True):
        with patch("ml_pipeline_monitor.ml.mlflow_tracker.load_config", side_effect=Exception("config error")):
            with patch("ml_pipeline_monitor.ml.mlflow_tracker.LOGGER") as mock_logger:
                log_pipeline_run(run_name="test", params={}, metrics={}, artifact_path="", model=MagicMock())
