...
def test_load_cached_splits_missing():
    with patch("ml_pipeline_monitor.ml.feature_store._feature_store_root", return_value=Path("/nonexistent")):
...
def test_save_and_load_cached_splits(tmp_path):
    ...
    with patch("ml_pipeline_monitor.ml.feature_store._feature_store_root", return_value=tmp_path):
