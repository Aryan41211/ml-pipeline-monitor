...
def test_save_model_tracks_version_and_lineage(monkeypatch, tmp_path):
    ...
    lineage = get_model_lineage(limit=10, dataset="Iris Species")
...
def test_update_model_stage_records_events_and_single_production(monkeypatch, tmp_path):
    ...
    update_model_stage("M1", "production")
    update_model_stage("M2", "production")

    latest_prod = get_latest_production_model(dataset="Wine Recognition")
    ...
    events_m1 = get_model_stage_events("M1", limit=20)
    events_m2 = get_model_stage_events("M2", limit=20)
